...
contract DynamicPriceNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function estimateCost(uint256 _supply) public view returns(uint256 _cost){
    ...
    //   else if(_supply <= (maxSupply * 50/100)) {
    //       return cost*2;
    //   }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintAmount, "Request Lower Mint Amount");
    require(supply + _mintAmount <= maxSupply, "Max Supply Exceeded");
    require(msg.value >= estimateCost(supply) * _mintAmount, "Not Enough Funds");
